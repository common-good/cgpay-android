...
public class WelcomeActivity extends BaseActivity {
	...
	protected void onStart()
	{
		...
		String finish_on_load = retriveSettings("finish_on_load", 1).toString();
		if(finish_on_load.equals("1"))
		{
			//Reset variable and finish
			saveSettings("finish_on_load", "", 1);
			finish();
		}
	...
	{
		//Checks from the preferences if this is the first run and
		//presents the appropriate screen to the user
		boolean retVal = false;
		
		String loginToken = String.valueOf(retriveSettings("loginToken", 1));
		if(loginToken.equals(""))
		{
			//Login token not set, this is the first run
			retVal = true;
		}
		//else the Login token exists, this is any other successive run
	...
	/**
	 * As the name suggest, prepares the interface to be used 
	 */
							...
							else
							{
								...
								else
								{
									...
									try
									{
								    	...
								    	saveSettings("update_link", str_update_link, 1);
								    	...
								    	if(str_success.equals("0"))
								    	{
								    		...
								    		if(str_message.equals("bad id") || str_message.equals("bad login"))
								    		{
								    			//Wrong username or password
								    			showDialogNegative(tag, "Wrong id or password, please try again");
								    		}
								    		else
								    		{
								    			//Just show the message which came from the server
								    	...
								    	else
								    	{
							    			...
							    			//Save settings
							    			saveSettings("loginToken", str_code, 1);
							    			saveSettings("my_id", str_id, 1);	//Default owner id or account id
	...
	{
		//Let's retrive the stored up code and the id first
		String str_my_id = String.valueOf(retriveSettings("my_id", 1));
		String str_code = String.valueOf(retriveSettings("loginToken", 1));

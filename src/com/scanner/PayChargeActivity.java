...
public class PayChargeActivity extends BaseActivity {
	//Variables
	String action_flag, my_id, code, data_from_scanner, account_id;
	...
	private void prepareInterface()
	{
		...
		{
			//It is implicitly assumed, that the other screen is for pay
		...
		btn_submit = (Button) findViewById(R.id.bntSubmit);
					...
					new Thread(new Runnable() {
			        	public void run() {
							...
							try
							{
								...
								apiParamsJsonTransact.put("type", action_flag);
								...
								List<NameValuePair> nameValuePairsTransact;
								nameValuePairsTransact = new ArrayList<NameValuePair>(1);
						    	// Adding parameters to send to the HTTP server.
								nameValuePairsTransact.add(new BasicNameValuePair("json", apiParamsJsonTransact.toString()));
						    	
						    	userDataResponse = makeAPICall(nameValuePairsTransact);
						    	...
						    	runOnUiThread(new Runnable() {
				        			public void run() {
								    	...
								    	if(str_success.equals("1"))
								    	{
								    		...
								    		saveSettings("tx_id", str_tx_id, 1);
								    		saveSettings("my_balance", str_my_balance, 1);
								    		saveSettings("other_balance", str_other_balance, 1);

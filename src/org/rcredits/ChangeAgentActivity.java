...
public class ChangeAgentActivity extends BaseActivity {
  // Local variables
  String action, data_from_scanner, str_my_id, str_account_id, str_code; 
  ...
  protected void onCreate(Bundle savedInstanceState) {
    ...
    setContentView(R.layout.layout_change_agent);
    ...
    str_my_id = getData("my_id").toString();
    str_code = getData("loginToken").toString();
  ...
  private void makeServerCallForChangeAgent()
  {
    ...
    mDialog = new ProgressDialog(ChangeAgentActivity.this);
...
//              apiParamsCA.put("my_id", str_my_id.toUpperCase());  // Though this is not mandatory from 4th Nov, 2012, but omitting it server returns "BAD ID FORMAT"
          ...
          apiParamsCA.put("what", "agent");
          apiParamsCA.put("op", "change");
          
          List<NameValuePair> pairs = new ArrayList<NameValuePair>(1);  

          pairs.add(new BasicNameValuePair("json", apiParamsCA.toString()));
          String changeAgentResp = makeAPICall(pairs);
          Log.e(tag, changeAgentResp);
          updateUI(changeAgentResp);
  ...
  private void updateUI(final String changeAgentResp)
  {
    // Update UI on a UI thread
    // Get message and all that from response
    final String str_status, str_message;
    str_status = getValueForJSONKey(changeAgentResp, "success");
    str_message = getValueForJSONKey(changeAgentResp, "message");
      ...
      public void run() {
        ...
        if(str_status.equals("0"))
        {
          ...
          // Need to be shown in confirmation dialogs
        ...
        else
        {
          ...
          String str_my_id = getValueForJSONKey(changeAgentResp, "my_id");
          putData("my_id", str_my_id);
